...
//		boolean isUser = userService.isUser(id,pw);
	...
	public ModelAndView list2Get(ModelAndView mv, Integer num, HttpServletRequest request) {
		ArrayList<UserVo> user = userService.getAllUser();
	...
	@ResponseBody
	//responseBody requestBody 필요
	//data를 보낼거라 modelandview 빼도 됨
	//return타입 object 아무나 들어오시오 모든 클래스들을 상곡하기 때문
	...
	public Object findPwPost(@RequestBody UserVo user) {
		...
		if(dbUser == null)
			 ...
			 * 랜덤으로 0~61 사이에 숫자를 뽑아서 0~9면 그냥 숫자로
			 * 10~35면 소문자 a~z로
			 * 36~61이면 대문자 A~Z로
			...
			for(int i = 0; i < 8; i++) {
				...
				if(r <= 9) {
					...
					ch = (char)('0' + r); // 문자 0 에 해당하는 아스키코드값+r => 문자+int >int화 error
				}else if(r <= 35) {
					//이해가 잘 안댐
